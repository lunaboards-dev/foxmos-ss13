...
#define trvh struct CByondValue
#define Runtime(msg) Byond_CRASH(msg)
...
static const struct CByondValue World = {
    .type = WORLD_D,
    .data = {.ref = 0}
};

static const struct CByondValue Globals = {
    .type = WORLD_D,
    .data = {.ref = 1}
};

inline CByondValue GetGlobal(const char * name) {
    CByondValue val;
    Byond_ReadVar(&Globals, name, &val);
    return val;
}

inline CByondValue GetWorld(const char * name) {
    CByondValue val;
    Byond_ReadVar(&Globals, name, &val);
    return val;
}
...
inline CByondValue FromStr(char * str) {
	CByondValue v;
	ByondValue_SetStr(&v, str);
	return v;
}
