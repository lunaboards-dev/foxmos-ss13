...
struct Tile
{
	...
	ByondValue turf_ref; // not managed because turf refcounts are very unimportant and don't matter
...
struct PlanetAtmosInfo
{
	ByondValue last_initial = ByondNull;
...
std::shared_ptr<GasMixture> &get_gas_mixture(ByondValue src);
